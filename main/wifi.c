...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data){
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
       esp_wifi_connect();
        ...
        ESP_LOGE(TAG, "Connecting wifi to ssid: %s pw: %s \n", CONFIG_WIFI_SSID, CONFIG_WIFI_PASSWORD);

        ESP_LOGI(TAG, "Wi-Fi reason codesX: https://docs.espressif.com/projects/esp-idf/en/v4.2/esp32/api-guides/wifi.html#wi-fi-reason-code");
        ESP_LOGE(TAG, "Wi-Fi error. Attempting reconnect...\n");

        xEventGroupClearBits(wifi_event_group, CONNECTED_BIT);
        xEventGroupSetBits(wifi_event_group, DISCONNECTED_BIT);
        uiWIFILabelUpdate(false);
        esp_wifi_connect();
